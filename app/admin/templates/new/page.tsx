...
function defaultDay(n: number): DietDay {
    return {
        day: n,
        meals: [
            { id: uid(), label: "Kahvaltı", items: [] },
            { id: uid(), label: "Öğle Yemeği", items: [] },
            { id: uid(), label: "Akşam Yemeği", items: [] },
        ],
        notes: "",
    };
}
...
function migrateDayFormat(d: any): DietDay {
    ...
    if (meals.length === 0) {
        meals.push({ id: uid(), label: "Kahvaltı", items: [] }, { id: uid(), label: "Öğle Yemeği", items: [] }, { id: uid(), label: "Akşam Yemeği", items: [] });
    }
...
export default function NewTemplatePage() {
    ...
    useEffect(() => {
        supabase.from('food_library').select('*').then(({ data }) => {
    ...
    const addMealAfter = (afterIdx: number) => {
        setDiet(prev => prev.map((d, i) => {
            if (i !== activeDay) return d;
            const meals = [...d.meals];
            meals.splice(afterIdx + 1, 0, { id: uid(), label: "Ara Öğün", items: [] });
            return { ...d, meals };
        }));
    };
    ...
    const removeMeal = (mealId: string) => {
        setDiet(prev => prev.map((d, i) => {
            if (i !== activeDay) return d;
            if (d.meals.length <= 1) return d;
            return { ...d, meals: d.meals.filter(m => m.id !== mealId) };
        }));
    };

    const renameMeal = (mealId: string, label: string) => {
        setDiet(prev => prev.map((d, i) => i !== activeDay ? d : { ...d, meals: d.meals.map(m => m.id === mealId ? { ...m, label } : m) }));
    };

    const addRow = (mealId: string) => {
        setDiet(prev => prev.map((d, i) => i !== activeDay ? d : { ...d, meals: d.meals.map(m => m.id === mealId ? { ...m, items: [...m.items, { food: "", portion: "", cal: 0 }] } : m) }));
    };

    const removeRow = (mealId: string, ri: number) => {
        setDiet(prev => prev.map((d, i) => {
            if (i !== activeDay) return d;
            return { ...d, meals: d.meals.map(m => { if (m.id !== mealId) return m; const items = [...m.items]; items.splice(ri, 1); return { ...m, items }; }) };
        }));
    };

    const handleFoodChange = (mealId: string, ri: number, value: string) => {
        const sel = foods.find(f => f.name.toLowerCase() === value.toLowerCase());
        setDiet(prev => prev.map((d, i) => {
            if (i !== activeDay) return d;
            return { ...d, meals: d.meals.map(m => { if (m.id !== mealId) return m; const items = [...m.items]; items[ri] = { ...items[ri], food: value }; if (sel) { items[ri].portion = sel.unit; items[ri].cal = sel.cal; } return { ...m, items }; }) };
        }));
    };

    const updateRow = (mealId: string, ri: number, field: string, value: any) => {
        setDiet(prev => prev.map((d, i) => {
            if (i !== activeDay) return d;
            return { ...d, meals: d.meals.map(m => { if (m.id !== mealId) return m; const items = [...m.items]; items[ri] = { ...items[ri], [field]: value }; if (field === "portion") { const base = foods.find(f => f.name.toLowerCase() === String(items[ri].food).toLowerCase()); if (base) { const c = calculateCalories(String(value), base); if (c !== null) items[ri].cal = c; } } return { ...m, items }; }) };
        }));
    };

    const handleNotesChange = (value: string) => {
        setDiet(prev => prev.map((d, i) => i === activeDay ? { ...d, notes: value } : d));
    };

    const handleSave = async () => {
        if (!templateTitle.trim()) return alert("Lütfen şablon adı girin.");
        setSaving(true);
        const { error } = await supabase.from('diet_templates').insert([{ title: templateTitle.trim(), description: templateDescription.trim(), content: diet }]);
        setSaving(false);
        if (error) alert("Kayıt hatası: " + error.message);
        else { alert("Şablon kaydedildi!"); router.push("/admin/templates"); }
    };
            ...
            <div style={{ flex: 1, padding: "50px", display: "flex", flexDirection: "column" }}>
                <div style={{ marginBottom: "35px" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "15px", flexWrap: "wrap", gap: "15px" }}>
                        <h1 style={{ fontSize: "26px", fontWeight: 700, color: "#2c3e50", margin: 0 }}>Yeni Şablon Oluştur</h1>
                ...
                {/* Şablon Bilgileri */}
                <div style={{ background: "white", padding: "24px", borderRadius: "14px", marginBottom: "24px", boxShadow: "0 4px 12px rgba(0,0,0,0.04)" }}>
                    <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
                        ...
                        <div style={{ flex: 3, minWidth: "200px" }}>
                            ...
                            <input value={templateDescription} onChange={e => setTemplateDescription(e.target.value)} placeholder="Bu şablon hakkında kısa not..." style={inputStyle} />
                        </div>
                    </div>
                </div>

                {/* Gün Sekmeleri */}
                <div style={{ display: "flex", gap: "10px", marginBottom: "22px", overflowX: "auto", paddingBottom: "6px", flexWrap: "wrap" }}>
                    {diet.map((_, idx) => (
                        <div key={idx} style={{ position: "relative" }}>
                            <button onClick={() => setActiveDay(idx)} style={{ padding: "10px 20px", background: activeDay === idx ? "#79a33d" : "white", color: activeDay === idx ? "white" : "#2c3e50", border: `1px solid ${activeDay === idx ? "#79a33d" : "#e0e0e0"}`, borderRadius: "10px", fontWeight: 700, cursor: "pointer" }}>
                                {idx + 1}. Gün
                            </button>
                            {diet.length > 1 && (
                                ...
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                                        {editingMealId === meal.id ? (
                                            ...
                                            <>
                                                <h3 style={{ fontSize: "16px", fontWeight: 700, color: "#3d5a2d", margin: 0 }}>{meal.label}</h3>
                                                <button onClick={() => setEditingMealId(meal.id)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "13px", color: "#bdc3c7", padding: "2px" }}>✏️</button>
                                            </>
                                        )}
                                    </div>
